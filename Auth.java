...
public class Auth {
    ...
    public static String getUrl(String api_id, String settings){
        String url= null;
        try {
            url = "https://oauth.vk.com/authorize?client_id="+api_id+"&display=mobile&scope="+settings+"&redirect_uri="+ URLEncoder.encode(redirect_url, "UTF_8")+"&response_type=token"
                    +"&v="+URLEncoder.encode(Api.API_VERSION, "UTF_8");
